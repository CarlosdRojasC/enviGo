...
const navigateToNext = () => {
  ...
  showNavigationOptions(nextStop.value)
}
  ...
  window.openNavigation = (app, lat, lng, addr) => {
    ...
    switch (app) {
      case 'google':
        ...
        if (addr) {
          url = `https://www.google.com/maps/dir/?api=1&destination=${encodeURIComponent(addr)}&destination_place_id=&travelmode=driving`
        } else {
