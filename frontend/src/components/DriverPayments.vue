...
export default {
  ...
  setup() {
    ...
    const loadPaymentReport = async () => {
      loading.value = true
      try {
        const response = await apiService.get(`/driver-history/company/${authStore.user.company_id}/monthly-report`, {
          params: {
    ...
    const confirmPayment = async () => {
      ...
      try {
        await apiService.post(`/driver-history/driver/${confirmData.value.driverId}/pay-all`, {
    ...
    const payAllDrivers = async () => {
      ...
      try {
        ...
        for (const driver of reportData.value.drivers) {
          await apiService.post(`/driver-history/driver/${driver._id}/pay-all`, {
            companyId: authStore.user.company_id
          })
        }
